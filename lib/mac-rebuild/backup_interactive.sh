...
if [ -d "$HOME/.ssh" ]; then
    # Check if SSH keys should be forced to exclude (public repo)
    if [[ "${FORCE_EXCLUDE_SSH:-}" == "true" ]]; then
        warn "SSH keys excluded (public repository mode)"
...
2. After fresh install, run: mac-rebuild restore <backup-path>
3. The restore will read your preferences and only restore what you selected
EOF
...
echo ""
echo "Next: After fresh install, run: mac-rebuild restore <backup-path>"
